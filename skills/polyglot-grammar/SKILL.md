---
name: polyglot-grammar
description: Teach and drill grammar patterns in any target language. Use for recurring grammatical errors, case/gender/tense/aspect/word-order/particles/register issues, sentence transformation, and grammar explanations tied to practice.
---

# Polyglot Grammar

## Role

Turn a grammar gap into a small mental model and immediate production practice. Grammar teaching must end in use, not explanation alone.

Before correcting learner output, follow `../polyglot-router/references/feedback-protocol.md`. For full sessions, follow `../polyglot-router/references/session-protocol.md`, `../polyglot-router/references/exercise-protocols.md`, and `../polyglot-router/references/rubrics.md`.

## Workflow

1. Name the pattern and the communicative job it does.
2. Give the smallest rule that helps the learner act correctly.
3. Contrast it with the learner's likely wrong pattern.
4. Run 3-7 drills: recognition, transformation, cloze, production, mixed review.
5. Record errors with a stable `pattern_id`.

## Exercise Sequence

Use this order unless the router gives a narrower task:

1. Recognition: choose or identify the correct form.
2. Controlled production: fill a blank or transform a sentence.
3. Contrast: fix a common wrong answer.
4. Free production: write one sentence using the pattern in a useful context.
5. Mixed retrieval: combine the pattern with one due review item when possible.

## Feedback Format

For each answer:

- Verdict and corrected version.
- One reason, phrased at the learner's level.
- A minimal pair or contrast when useful.
- Next micro-drill.

## Rules

- Use the learner's native language for explanation unless immersion mode is active.
- Use target language examples that match the learner's level plus one step.
- For languages with politeness/register systems, mark register explicitly.
- Never invent a universal grammar rule when the language has major dialect or register variation.
